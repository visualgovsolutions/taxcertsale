...
interface User {
    id: string;
    role: 'ADMIN' | 'COUNTY_OFFICIAL' | 'INVESTOR' | 'USER'; 
}
...
const AdminDashboardPage: React.FC = () => {
  const { loading: loadingAuctions, error: errorAuctions, data: dataAuctions } = useQuery<GetActiveAuctionsData>(GET_ACTIVE_AUCTIONS);
  const { loading: loadingCounties, error: errorCounties, data: dataCounties } = useQuery<GetCountiesData>(GET_COUNTIES);
  const { loading: loadingUsers, error: errorUsers, data: dataUsers } = useQuery<GetUsersData>(GET_USERS);
  const { loading: loadingUpcoming, error: errorUpcoming, data: dataUpcoming } = useQuery<GetUpcomingAuctionsData>(GET_UPCOMING_AUCTIONS);
  const { loading: loadingRegToday, error: errorRegToday, data: dataRegToday } = useQuery<GetUsersRegisteredTodayData>(GET_USERS_REGISTERED_TODAY_COUNT);

  const countyCount = dataCounties?.counties?.length;
  const bidderCount = dataUsers?.users?.filter(user => user.role === 'INVESTOR').length;
  ...
  return (
    <div className="p-4">
      ...
      {/* Grid for Widgets */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 mt-4"> {/* Adjusted columns for more widgets */}
